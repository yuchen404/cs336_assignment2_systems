...
from annotated_ScaledDotProdAttention import annotated_scaled_dot_product_attention
...
def parse_args():
    ...
    parser.add_argument("--mode", default="forward_backward", choices=["forward_only", "forward_backward"])
...
def benchmarking():
            ...
            with precision_context:
                if mode == "forward_only":
                    with nvtx.range("forward"):
                        logits = model(input_x)
                    torch.cuda.memory._dump_snapshot(f"memory_profile/fwd-memory-snapshot.pickle")
    ...
    torch.cuda.memory._dump_snapshot("memory_profile/memory_snapshot_full.pickle")
